...
if (!supabaseUrl || supabaseUrl.includes('your-project') || !supabaseAnonKey || supabaseAnonKey.includes('your_anon_key')) {
...
export const supabaseHelpers = {
  ...
  async updateBusCapacityStatus(busId, capacityPercentage, pwdPassengers = null) {
    console.log('📡 Updating bus capacity status for bus:', busId, 'capacity:', capacityPercentage + '%', 'PWD:', pwdPassengers);
    ...
    if (pwdPassengers !== null && pwdPassengers < 0) {
      throw new Error('PWD passengers cannot be negative');
    }
    ...
    try {
      // Get capacity and PWD data from bus record first
      const { data: busData, error: fetchError } = await supabase
        .from('buses')
        .select('capacity, pwd_seats, current_pwd_passengers, pwd_seats_available')
        .eq('id', busId)
        .single();
      ...
      // Get PWD seats info (handle case where columns might not exist)
      const pwdSeats = busData?.pwd_seats || 4;
      let pwdSeatsAvailable;
      
      if (pwdPassengers !== null) {
        // Calculate available seats based on provided PWD passengers
        pwdSeatsAvailable = Math.max(0, pwdSeats - pwdPassengers);
      } else {
        // Use existing data or calculate from current PWD passengers
        pwdSeatsAvailable = busData?.pwd_seats_available !== undefined
          ? busData.pwd_seats_available
          : Math.max(0, pwdSeats - (busData?.current_pwd_passengers || 0));
      }
      ...
      // Add PWD data if provided (only if columns exist - will fail gracefully if they don't)
      if (pwdPassengers !== null) {
        updateData.current_pwd_passengers = pwdPassengers;
        // Don't set pwd_seats_available explicitly - let the database trigger calculate it
        // This ensures consistency and avoids conflicts with the trigger
        // The trigger will automatically calculate: pwd_seats_available = pwd_seats - current_pwd_passengers
        // Also ensure pwd_seats is set if it's missing
        if (!updateData.pwd_seats && pwdSeats) {
          updateData.pwd_seats = pwdSeats;
        }
      }
      ...
      console.log('📤 Sending update to database:', {
        ...
        updateData,
        pwdPassengers,
        expectedPwdSeatsAvailable: pwdSeatsAvailable, // Expected value (will be calculated by trigger)
        pwdSeats
      });

      // Try using the RPC function first (bypasses RLS)
      // If it doesn't exist, fall back to direct update
      let data, error;
      
      try {
        console.log('🔄 Attempting RPC function call...');
        const rpcResult = await supabase.rpc('update_bus_capacity', {
          p_bus_id: busId,
          p_capacity_percentage: capacityPercentage,
          p_current_passengers: currentPassengers,
          p_current_pwd_passengers: pwdPassengers !== null ? pwdPassengers : null
        });
        
        console.log('📡 RPC result:', { data: rpcResult.data, error: rpcResult.error });
        
        if (rpcResult.error) {
          console.warn('⚠️ RPC function error:', rpcResult.error);
          ...
          if (rpcResult.error.message && (
            ...
            throw new Error('RPC_FUNCTION_NOT_FOUND'); // Special error to trigger fallback
          }
          // For other RPC errors, still try direct update
          throw rpcResult.error;
        }
        
        // Handle different return types (array, object, or JSON)
        if (rpcResult.data) {
          if (Array.isArray(rpcResult.data) && rpcResult.data.length > 0) {
            data = rpcResult.data[0];
            error = null;
            console.log('✅ Update via RPC function successful (array):', data);
          } else if (!Array.isArray(rpcResult.data) && typeof rpcResult.data === 'object') {
            // Function returns a single object (JSON)
            data = rpcResult.data;
            error = null;
            console.log('✅ Update via RPC function successful (object):', data);
          } else {
            console.warn('⚠️ RPC function returned unexpected data format:', rpcResult.data);
            throw new Error('RPC_FUNCTION_NOT_FOUND');
          }
        } else {
          console.warn('⚠️ RPC function returned no data, falling back to direct update');
          throw new Error('RPC_FUNCTION_NOT_FOUND');
        }
      } catch (rpcError) {
        // Fall back to direct update if RPC function doesn't exist or fails
        if (rpcError.message === 'RPC_FUNCTION_NOT_FOUND' || rpcError.code === '42883') {
          console.log('📤 RPC function not available, using direct update method');
        } else {
          console.log('📤 RPC call failed, falling back to direct update:', rpcError);
        }
        ...
        console.log('📡 Direct update result (without select):', { 
        ...
        if (updateResult.error) {
          if (updateResult.error.code === '42501' || updateResult.error.message?.includes('permission denied') || updateResult.error.message?.includes('policy')) {
            console.error('❌ RLS POLICY BLOCKING UPDATE:', updateResult.error);
            throw new Error(`RLS Policy Error: ${updateResult.error.message}. Make sure you ran sql/force-enable-bus-updates.sql`);
          }
        }
        
        // Check if update succeeded - Supabase returns empty data on success without select
        // If there's no error, the update likely succeeded (even if data is null)
        const updateSucceeded = !updateResult.error;
        
        if (updateSucceeded) {
          console.log('✅ Update command succeeded (no error), fetching updated data...');
          const fetchResult = await supabase
            .from('buses')
            .select('id, current_passengers, capacity_percentage, current_pwd_passengers, pwd_seats_available, pwd_seats, capacity')
            .eq('id', busId)
            .single();
          
          console.log('📡 Fetch result after update:', { data: fetchResult.data, error: fetchResult.error });
          
          if (fetchResult.error) {
            console.warn('⚠️ Update succeeded but failed to fetch updated data:', fetchResult.error);
            console.warn('⚠️ This might be due to RLS SELECT policies, but the update likely worked');
            // Don't treat this as a fatal error - the update probably succeeded
            // Return the data we tried to update as confirmation
            data = {
              id: busId,
              capacity_percentage: capacityPercentage,
              current_passengers: currentPassengers,
              current_pwd_passengers: pwdPassengers !== null ? pwdPassengers : undefined,
              pwd_seats_available: pwdPassengers !== null ? pwdSeatsAvailable : undefined,
              pwd_seats: pwdSeats,
              capacity: maxCapacity,
            };
            error = null; // Don't treat fetch failure as update failure
            console.log('✅ Returning update data as confirmation (fetch failed but update likely succeeded)');
          } else {
            data = fetchResult.data;
            error = null;
          }
        } else {
          // Update itself failed - there's an error
          error = updateResult.error;
          data = null;
          ...
          if (updateResult.error?.code === '42501' || updateResult.error?.message?.includes('permission')) {
            error = new Error(`Permission denied: ${updateResult.error.message}. Make sure you ran sql/force-enable-bus-updates.sql`);
          } else if (updateResult.error) {
            error = new Error(`Update failed: ${updateResult.error.message || 'Unknown error'}`);
          } else {
            error = new Error('Update failed with no error message. This usually means RLS policies are blocking the update.');
          }
        }
      }
      ...
      if (error) {
        ...
        const isColumnError = errorMessage.includes('capacity_percentage') || 
                            ...
                            errorMessage.includes('current_pwd_passengers') ||
                            errorMessage.includes('pwd_seats') ||
                            errorMessage.includes('does not exist') ||
                            errorMessage.includes('schema cache');
        
        if (isColumnError) {
          const missingPwdColumns = errorMessage.includes('pwd') || errorMessage.includes('PWD');
          throw new Error(
            ...
            (missingPwdColumns ? '\n\nNote: PWD columns (pwd_seats, current_pwd_passengers, pwd_seats_available) may also need to be added.' : '')
          );
        }
      ...
      if (!data) {
        console.warn('⚠️ Update succeeded but returned no data - this might be okay');
        // Try to fetch the updated data manually
        ...
        if (fetchError) {
          console.warn('⚠️ Failed to fetch updated data, but update may have succeeded:', fetchError);
          // Return the data we tried to update as confirmation
          // This way the app knows the update was sent, even if we can't verify it
          data = {
            id: busId,
            capacity_percentage: capacityPercentage,
            current_passengers: currentPassengers,
          ...
          console.log('✅ Returning update confirmation (could not verify but update was sent)');
        } else if (fetchedData) {
          console.log('✅ Fetched updated data after update:', fetchedData);
          data = fetchedData;
        } else {
          // Still return confirmation data
          data = {
            id: busId,
            capacity_percentage: capacityPercentage,
            current_passengers: currentPassengers,
          ...
          console.log('✅ Returning update confirmation (no data fetched but update was sent)');
        }
      }
      
      console.log('✅ Bus capacity updated successfully:', data);
      console.log('✅ PWD data in response:', {
        current_pwd_passengers: data.current_pwd_passengers,
        pwd_seats_available: data.pwd_seats_available,
        pwd_seats: data.pwd_seats
      });
      
      // Verify the update actually worked
      if (pwdPassengers !== null) {
        if (data.current_pwd_passengers !== pwdPassengers) {
          console.warn('⚠️ WARNING: PWD passengers mismatch! Expected:', pwdPassengers, 'Got:', data.current_pwd_passengers);
        }
        if (data.pwd_seats_available !== undefined && data.pwd_seats_available !== pwdSeatsAvailable) {
          console.warn('⚠️ WARNING: PWD seats available mismatch! Expected:', pwdSeatsAvailable, 'Got:', data.pwd_seats_available);
        }
      }
  ...
  async getBusCapacityStatus(busId) {
    ...
    try {
      // Fetch capacity and PWD data
      const { data, error } = await supabase
        .from('buses')
        .select('id, current_passengers, capacity_percentage, capacity, pwd_seats, current_pwd_passengers, pwd_seats_available')
        .eq('id', busId)
        .single();
      ...
      if (error) {
        ...
        return {
          ...
          pwd_seats: 4,
          current_pwd_passengers: 0,
          pwd_seats_available: 4,
        };
      }

      // Map 'capacity' to 'max_capacity' for backward compatibility
      const pwdSeats = data.pwd_seats || 4;
      const currentPwdPassengers = data.current_pwd_passengers || 0;
      const pwdSeatsAvailable = data.pwd_seats_available !== undefined 
        ? data.pwd_seats_available 
        : Math.max(0, pwdSeats - currentPwdPassengers);

      return {
        ...data,
        max_capacity: data.capacity || 50, // For backward compatibility
        pwd_seats: pwdSeats,
  ...
  async authenticateDriver(email, password) {
    ...
    try {
      ...
      if (fetchError) {
        ...
        // Handle JSON parse errors and connection issues
        if (fetchError.message && (
          fetchError.message.includes('JSON Parse error') ||
          fetchError.message.includes('Unexpected character') ||
      ...
      if (error?.message) {
        if (error.message.includes('JSON Parse error') || 
          ...
          throw new Error('Connection error. Please check your internet connection and verify your Supabase configuration.');
  ...
  async acknowledgePing(pingId) {
    ...
    try {
      ...
      // The RPC function returns a boolean (true if update was successful, false if no rows were updated)
      if (data === true) {
        console.log('✅ Ping acknowledged successfully');
        return { success: true, data };
      } else {
        console.warn('⚠️ Ping not found or already acknowledged');
        return { success: false, error: 'Ping not found or already acknowledged' };
      }
    } catch (error) {
      console.error('❌ Error acknowledging ping:', error);
      return { success: false, error: error.message };
  ...
  async completePing(pingId) {
    ...
    try {
      ...
      if (data === true) {
        ...
        console.warn('⚠️ Ping not found or cannot be completed');
        return { success: false, error: 'Ping not found or cannot be completed' };
      }
    } catch (error) {
      console.error('❌ Error completing ping:', error);
      return { success: false, error: error.message };
